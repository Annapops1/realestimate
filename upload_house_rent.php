...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate inputs
    if (empty($place) || empty($state) || empty($district) || empty($photos) || empty($size) || empty($bedrooms) || empty($bathrooms)) {
    ...
    if (!is_numeric($bedrooms) || $bedrooms < 1) {
        $errorMessages[] = "Bedrooms must be a positive integer.";
    }
    if (!is_numeric($bathrooms) || $bathrooms < 1) {
        $errorMessages[] = "Bathrooms must be a positive integer.";
    }

    if (empty($errorMessages)) {
        // Check for duplicate properties
        $checkSql = "SELECT * FROM properties WHERE place = ? AND price = ? AND state = ? AND district = ? AND size = ? AND bedrooms = ? AND bathrooms = ?";
        $checkStmt = $conn->prepare($checkSql);
        $checkStmt->bind_param("ssssssi", $place, $price, $state, $district, $size, $bedrooms, $bathrooms);
        $checkStmt->execute();
        ...
        if ($checkResult->num_rows > 0) {
            ...
            $sql = "INSERT INTO properties (user_id, place, price, photo, property_type, transaction_type, state, district, size, bedrooms, bathrooms) 
                    VALUES (?, ?, ?, ?, 'house', 'rent', ?, ?, ?, ?, ?)";
            ...
            $stmt->bind_param("issssssss", $_SESSION['user_id'], $place, $price, $photo_string, $state, $district, $size, $bedrooms, $bathrooms);
...
<header>
    <h1>Upload House for Rent</h1>
</header>
<div class="upload-container">
    <form action="upload_house_rent.php" method="post" enctype="multipart/form-data">
        <!-- State Dropdown -->
...
<script>
    ...
    // Client-side validation for size, price, bedrooms, and bathrooms
    document.getElementById('size').addEventListener('input', function() {
        ...
        if (sizeInput === '' || isNaN(sizeInput) || sizeInput < 5) {
            sizeError.textContent = 'Size must be at least 5 cent and a positive number.';
        }
    });
    ...
    document.getElementById('bedrooms').addEventListener('input', function() {
        const bedroomsInput = this.value;
        const bedroomsError = document.getElementById('bedroomsError');
        bedroomsError.textContent = ''; // Clear previous error message

        if (bedroomsInput === '' || isNaN(bedroomsInput) || bedroomsInput < 1) {
            bedroomsError.textContent = 'Bedrooms must be greater than zero.';
        }
    });

    document.getElementById('bathrooms').addEventListener('input', function() {
        const bathroomsInput = this.value;
        const bathroomsError = document.getElementById('bathroomsError');
        bathroomsError.textContent = ''; // Clear previous error message

        if (bathroomsInput === '' || isNaN(bathroomsInput) || bathroomsInput < 1) {
            bathroomsError.textContent = 'Bathrooms must be greater than zero.';
        }
    });
</script>
</body>
