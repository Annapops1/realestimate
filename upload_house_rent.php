...
$states = [];
$sql = "SELECT DISTINCT state FROM properties";
...
if (isset($_POST['state'])) {
    $state = $_POST['state'];
    $sql = "SELECT DISTINCT district FROM properties WHERE state = ?";
...
if (isset($_POST['district'])) {
    ...
    $sql = "SELECT DISTINCT place FROM properties WHERE district = ?";
    $stmt = $conn->prepare($sql);
...
VALUES (?, ?, ?,?, ?, ?, 'house', 'rent', ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iissssss", $_SESSION['user_id'], $place, $bedrooms, $bathrooms, $price, $photo, $state, $district);
